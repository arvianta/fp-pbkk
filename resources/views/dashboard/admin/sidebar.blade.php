<aside id="cta-button-sidebar" class="fixed top-0 left-0 z-0 w-1/6 pt-10 mt-10 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
    <div class="h-full px-3 py-4 overflow-y-auto bg-gray-50 border-r border-gray-300">
        <ul class="space-y-3 mt-2 mx-1 font-medium">
            ...
            <li>
                <a href="{{ route('admin.trainers') }}" class="flex items-center p-2 {{ request()->routeIs('admin.trainers') ? 'bg-primary text-dark3' : 'hover:bg-gray-200 hover:text-gray-900 group' }} text-gray-700 rounded-lg group">
                <svg class="w-5 h-5 transition duration-75 {{ request()->routeIs('admin.trainers') ? 'text-gray-900' : 'text-gray-500 group-hover:text-gray-800' }}"" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
